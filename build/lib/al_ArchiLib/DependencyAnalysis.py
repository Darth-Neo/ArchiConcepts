...
class DependencyAnalysis(object):
    ...
    def dependancyAnalysis(self):
            ...
            if al.dictEdges[x].has_key("source"):
                ...
                if al.dictEdges[x][AL.ARCHI_TYPE] in ("archimate:FlowRelationship"):
                    ...
                    if (al.dictNodes[source][AL.ARCHI_TYPE] == "archimate:BusinessProcess") and \
                        ...
                        sc = self.findConcept(concepts, sourceName)
                        if sc == None:
                        ...
                        tc = self.findConcept(concepts, targetName)
                        if tc == None:
        ...
        self.conceptBatches = Concepts("Batch", "archimate:WorkPackage")
        ...
        for bundle in batches:
            n += 1
            name = "Batch %d" % n
            c = self.conceptBatches.addConceptKeyType(name, "archimate:WorkPackage")
            for node in bundle:
                c.addConceptKeyType(node.name, "archimate:BusinessProcess")

            logger.info("%d : %s" % (n, ", ".join(node.name.lstrip() for node in bundle)))

        Concepts.saveConcepts(self.conceptBatches, AL.fileConceptsBatches)

        return self.conceptBatches

if __name__ == "__main__":
    concepts = DependencyAnalysis()
